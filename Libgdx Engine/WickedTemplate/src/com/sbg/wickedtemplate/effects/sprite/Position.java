...
public class Position extends Effect {
	...
	public Position(Object s, Effect e) {
		super(e);
		...
		if (s instanceof Group) {
			...
			timeline.push(Tween.set(s, GroupAccessor.POSITION).target(startValueA, startValueB))
					.push(Tween.to(s, GroupAccessor.POSITION, duration).target(endValueA, endValueB));
		} else {
			s = (ESprite) s;
			timeline.push(Tween.set(s, SpriteAccessor.POSITION).target(startValueA, startValueB))
					.push(Tween.to(s, SpriteAccessor.POSITION, duration).target(endValueA, endValueB));
		}
